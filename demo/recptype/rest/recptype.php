 ...
 *	recptype.php
 *
 *	This procedure is a REST compliant service for the recptype resource.  
 *	It will facilitate interaction with the PubAssist mySQL data source.
 *
 */

// Anything that is unique to the recptype resource should be declared in
// the recptypeRequest sub-class.
class recptypeRequest extends request{
	...
	function __construct(){
		...
		$this->resource = "recptype";
		$this->keyField = "id";
		
		//	Load the SQL templates for each request method to be supported.
		//	Unsupported methods should be left blank.
		$this->addMethodSQL('COUNT', 'recptype_COUNT.sql');
		$this->addMethodSQL('GET', 'recptype_GET.sql');
		...
		$this->addMethodSQL('DELETE', 'recptype_DELETE.sql');	

	}
}


$REST = new recptypeRequest();
