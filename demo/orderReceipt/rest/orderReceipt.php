 ...
 *	orderReceipt.php
 *
 *	This procedure is a REST compliant service for the orderReceipt resource.  
 *	It will facilitate interaction with the PubAssist mySQL data source.
 *
 */

// Anything that is unique to the orderReceipt resource should be declared in
// the orderReceiptRequest sub-class.
class orderReceiptRequest extends request{
	...
	function __construct(){
		...
		$this->resource = "orderReceipt";
		$this->keyField = "id";
		
		//	Load the SQL templates for each request method to be supported.
		//	Unsupported methods should be left blank.
		$this->addMethodSQL('COUNT', 'orderReceipt_COUNT.sql');
		$this->addMethodSQL('GET', 'orderReceipt_GET.sql');
		...
		$this->addMethodSQL('DELETE', 'orderReceipt_DELETE.sql');	

	}
}


$REST = new orderReceiptRequest();
