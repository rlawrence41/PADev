/* 
	These procedures are unique to the customer order transaction application.
...
// Global variables for the customer order transaction...
var orderObj = null;
...
/*
	addParent() generates a template customer order record and adds it to the Orders 
 ...
 */	
	...
	alert("About to create a customer order record with:  \n" + jsonStr) ;

	var url = location.protocol + "//"  + window.location.hostname + "/customerOrder/rest/customerOrder.php" ;
...
/*
	getOrder() retrieves the parent customer order record via the REST API.
 */
...
/*
	postProcessCustomerOrder() kicks off the post-process for the transaction via the 
	REST API.
...
function postProcessCustomerOrder(orderKey){
	
	// 	Sample REST url:
	//	https://dev.pubassist.com/orders/rest/orders.php/orders/3
	
	var url = location.protocol + "//" + location.hostname ;
	url += "/customerOrder/rest/customerOrder.php/customerOrder/" + orderKey;
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4) { 
			...
			if (this.status == 200) {
				alert("This customer order has been successfully processed.");
				location.assign("customerOrderAdmin.php");
			}
			else {
				alert("There was a problem processing this customer order: \n" +
					this.responseText);
			}
		} 
	}
...
function postReceiptForOrder(orderKey, customerNo, amount, status){
	...
	var receiptRestUri = restURL.replace(/customerOrder/g,"receiptDetail");
...
/*
	updateCustomerOrder() updates the orders record based on changes to any of
	the supporting resources.

 */
function updateCustomerOrder(orderKey=0) {
		
	var url = location.protocol + "//"  + window.location.hostname + 
				"/customerOrder/saveCustomerOrder.php" ;
...
/*
	...
	Thus the introduction of saveShipTo.php in the customerOrder application.
 */
 ...
 */
function updateParent(orderKey, jsonStr) {
