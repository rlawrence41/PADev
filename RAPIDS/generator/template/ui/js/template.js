/* 
	These procedures are unique to the <<resource>> transaction application.
...
/*
	addParent() generates a template parent record and adds it to the <<resource>> 
	resource--generating a new key.  The new transaction appears in the table.
 */ 

function addParent(){

	var url = location.protocol + "//"  + window.location.hostname + "/<<resource>>/rest/<<resource>>.php" ;
...
/*
	get<<parentResource>>() retrieves the parent <<parentResource>> record via the REST API.
 */

function get<<parentResource>>(<<parentResource>>No){
	
	// 	Sample REST url:
	//	https://dev.pubassist.com/orders/rest/orders.php/orders/3
	
	var url = location.protocol + "//" + location.hostname ;
	url += "/<<parentResource>>/rest/<<parentResource>>.php/<<parentResource>>/" + <<parentResource>>No;
