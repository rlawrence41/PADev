...
#		$protocol = ($_SERVER['HTTPS']=="on") ? "https://" : "http://" ;
		...
		// Either the application is located in the resource or the root folder.
		$path = $_SERVER['DOCUMENT_ROOT'] . "/" . $resource ;
		if ($this->folderExists($path)) {
			debug_msg("Yes!  Folder exists: " . $path, false, "GetAppUrl") ;
		...
		if ($this->folderExists($path)) {
			...
			debug_msg("No, folder does NOT exist: " . $path, false, "GetAppUrl") ;
			$appURL = $hostURL . "/{$resource}" ;
		}
		return $appURL ;
	}
	...
	public function getRESTurl($resource) {
		...
		// Either the REST API is located in the application or the common folder.
		$path = $_SERVER['DOCUMENT_ROOT'] . $resource ;
		if ($this->folderExists($path)) {
			debug_msg("Yes!  Folder exists: " . $path, false, "GetRESTurl") ;
			$restURL = $hostURL . "/{$resource}/rest/{$resource}.php/{$resource}" ;
		}
		else {
			debug_msg("No, folder does NOT exist: " . $path, false, "GetRESTurl") ;
			$restURL = $hostURL . "/common/rest/{$resource}.php/{$resource}" ;
		}
		return $restURL ;
	}
